...
class RPSImageLoader:
    ...
    def load_images(self):
        ...
        self.image_YouDefault = Image.open("You-Default-IMage.png").resize((200, 275))
...
class RPSGame:
    def __init__(self, window):
        ...
        self.background_animation = pygame.image.load("design_combine(2).gif").convert()
    ...
    def setup_ui(self):
        ...
        self.label_bot = Label(self.window, image=self.image_loader.image_BotDefault)
        self.label_bot.place(x=1000, y=200)
        self.bot_indicator = Label(self.window, text="BOT", bg="white", fg="#DE1884", font=("arial", 20, "bold")).place(x=1075, y=160)
        self.label_player = Label(self.window, image=self.image_loader.image_YouDefault)
        self.label_player.place(x=200, y=200)
        self.player_indicator = Label(self.window, text="PLAYER", bg="white", fg="#DE1884",font=("arial", 20, "bold")).place(x=250, y=160)

        # Initializing buttons for rock, paper, and scissors 
        self.button_rock = Button(self.window, borderwidth=0, bg="white", image=self.image_loader.image_rock,command=lambda: self.on_button_click("rock"))
        self.button_rock.place(x=400, y=588)
        self.button_paper = Button(self.window, borderwidth=0, bg="white", image=self.image_loader.image_paper,command=lambda: self.on_button_click("paper"))
    ...
    def setup_ui1(self):
        ...
        self.bot_score_label = Label(self.window, text=str(self.bot_score), font=("arial", 20, "bold"), bg="white",fg="#663399")
        self.bot_score_label.place(x=1100, y=498)

        # Initializing label to display final message
        self.final_message = Label(self.window, font=("arial", 20, "bold"), bg="white", fg="#FFCC33")
        self.final_message.place(x=625, y=550)
        
        # Initializing celebration labels (hidden)
        self.label_celebrate_player = Label(self.window, image=self.image_loader.image_celebrate_player, bg="white")
        self.label_celebrate_bot = Label(self.window, image=self.image_loader.image_celebrate_bot, bg="white")
        self.label_celebrate_tie = Label(self.window, image=self.image_loader.image_celebrate_tie, bg="white")
    ...
    def load_gif_animation(self, filename, x, y):
        ...
        self.gif_label = self.canvas.create_image(x, y, anchor=NW, image=self.gif_frames[0])
    ...
    def animate_gif(self):
        ...
        self.canvas.itemconfig(self.gif_label, image=self.gif_frames[self.gif_index])
